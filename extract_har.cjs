...
if (process.argv.length < 4) {
  console.error('Usage: node extract_har.cjs <har-file> <match-text>');
  process.exit(2);
}
...
matched.forEach((e, i) => {
  ...
  console.log('Request URL:', e.request && e.request.url);
  console.log('Method:', e.request && e.request.method);
  ...
  if (e.request && e.request.postData && e.request.postData.text) {
